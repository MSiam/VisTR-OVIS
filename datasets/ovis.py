...
def build(image_set, args):
    ...
    assert root.exists(), f'provided OVIS path {root} does not exist'
    mode = 'instances'
    PATHS = {
        "train": (root / "train", root / "annotations" / f'annotations_train.json'),
        "val": (root / "valid", root / "annotations" / f'annotations_valid.json'),
    }
    img_folder, ann_file = PATHS[image_set]
    dataset = OVISDataset(img_folder, ann_file, transforms=make_coco_transforms(image_set), return_masks=args.masks, num_frames = args.num_frames)
    return dataset
